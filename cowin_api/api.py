...
class CoWinAPI(BaseApi):
    ...
    def get_availability_by_district(self, district_id: str, date: str = today(), min_age_limit: int = None, available_capacity_gte: int = 0):
        url = f"{Constants.availability_by_district_url}?district_id={district_id}&date={date}"
        return filter_centers_by_age_limit(self._call_api(url), min_age_limit, available_capacity_gte) if min_age_limit else self._call_api(url)

    def get_availability_by_pincode(self, pin_code: str, date: str = today(), min_age_limit: int = None, available_capacity_gte: int = 0):
